...
SavePNG <- function(df, year){
  p <- df[,5]*100  
  prev <- matrix(p, nrow=length(bprev.vec), ncol=length(d.vec))
  colmat <- MapColors(prev)
  
  png(paste0("shinyApp/images/prevplot", year, ".png"), width = 7, height = 7, units="in", 
      res = 200)
  persp(bprev.vec, d.vec, prev, phi=25, theta=220,
        xlab="\n Baseline prevalence (%)", ylab="\n Renter selectivity", 
        zlab = "\n Year-end prevalence (%)", ticktype="detailed", border=NA, col=colmat,
        main = paste("Prevalence at the End of Year", year), cex.main=1.8,
        zlim=c(0, 10))
  dev.off()
}
